...
from utils.utils import set_random_seed, convert_to_gpu, load_patent_dataset, get_n_params
from utils.EarlyStopping import EarlyStopping
from utils.utils import load_dataset, get_node_data_loader, get_optimizer_and_lr_scheduler
from model.RGCN.RGCN import RGCN
from utils.Classifier import Classifier

args = {'dataset': 'patent',
        'model_name': 'RGCN_Temp',
        'embedding_name': 'emb_bert_hgnnI_2',
        'mode': 'train',
        'seed': 0,
        'predict_category': 'patent',
        'cuda': 1,
        'learning_rate': 0.001, # todo origin 0.001 0.0005
        ...
        'use_self_loop': True,
        'batch_size': 2560,
        'node_neighbors_min_num': 10, #todo origin:10 15
        'optimizer': 'adam', 'weight_decay': 0, 'epochs': 500, 'patience': 50} # todo epoch origin:500 1000
...
args["truth_path"] = root + "com_real_truth.json"
args['data_path'] = root + args['embedding_name'] + ".pkl"

# args['data_path'] = root + 'bert_raw.pkl'
# args['data_path'] = "/home/huangyf/PreData/R-HGNN-master-2/dataset/goodread/2_raw.pkl"
args['data_path'] = "/home/huangyf/PreData/R-HGNN-master-2/dataset/patent_mlp_mlm2/s_2.pkl"

# args['data_path'] = "/home/huangyf/PreData/R-HGNN-master-2/dataset/patent_mlm_neigh/2.pkl"
# todo
...
if __name__ == '__main__':
    ...
    fh = logging.FileHandler(f"../log/{args['model_name']}/{args['dataset']}/"
                             f"emb_{args['embedding_name']}_hidden_units_{args['hidden_units']}_seed_{args['seed']}_lr_{args['learning_rate']}_dp_{args['dropout']}-{time.time():.4f}.log")
    ...
    #     f"emb_{args['embedding_name']}_hidden_units_{args['hidden_units']}_seed_{args['seed']}_lr_{args['learning_rate']}_dp_{args['dropout']}",
    #     args, run_dir)
    ...
    if args['mode'] == 'train':
        for epoch in range(args['epochs']):
            ...
            for batch, (input_nodes, output_nodes, blocks) in enumerate(train_loader_tqdm):
                ...
                # config_saver.record({"loss": loss.item()}, 0)
            ...
            # config_saver.record({"loss": val_total_loss}, 1)
            # config_saver.record(val_scores, 1)
            ...
            test_scores = get_metric(y_true=test_y_trues, y_pred=test_y_predicts.detach().cpu(),
                                     method='macro', stage='valid')
            # config_saver.record({"loss":test_total_loss}, 2)
            # config_saver.record(test_scores, 2)
            logger.info(
        ...
        params = torch.load("../save_model/patent/RGCN/bert_hgnnI_0/RGCN.pkl", map_location='cpu')
    ...
    # config_saver.record(train_scores, 0)
