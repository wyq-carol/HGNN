...
sys.path.append('/raid/wangyiqing/workspace/HGNN/HGNN/utils')

from utils.utils import set_random_seed, convert_to_gpu, load_patent_dataset
from utils.EarlyStopping import EarlyStopping
from utils.utils import get_n_params, get_edge_data_loader, get_predict_edge_index, get_optimizer_and_lr_scheduler, \
    evaluate_link_prediction, add_neighbor
...
args = {
    'dataset': 'patent',
    'model_name': 'HGCN_lr0.001_dropout0.3_seed_0_link_prediction_mean',
    ...
    'norm': True,
    'use_self_loop': True,
    'batch_size': 1280,  # the number of edges to train in each batch
    'node_neighbors_min_num': 10,  # number of sampled edges for each type for each GNN layer
    'negative_sample_edge_num': 5,
    'sample_edge_rate': 0.02,  # train: validate: test = 3 : 1 : 1
    'sampled_edge_type': 'applicant_company',  # writes or affiliated_with, two kinds of predicted relations
    'optimizer': 'adam',
    'weight_decay': 0.0,
    'epochs': 500,
    'patience': 20,
    'add_patent_neighbor': True
}
root = '/home/share/Patent/COM-1000/'

args['data_path'] = "/home/huangyf/PreData/R-HGNN-master-2/dataset/1.pkl"
args['data_path'] = '/home/huangyf/PreData/R-HGNN-master-2/dataset/patent_mlm_neigh/0.pkl'

args['data_split_idx_path'] = root + 'split_idx.pkl'
...
def evaluate(model: nn.Module, loader: dgl.dataloading.NodeDataLoader, loss_func: nn.Module,
    ...
    with torch.no_grad():
        ...
        for batch, (input_nodes, positive_graph, negative_graph, blocks) in enumerate(loader_tqdm):
            ...
            filter_representation = {"applicant": nodes_representation["applicant"],
                                     "company": nodes_representation["company"]}

            positive_score = model[1](positive_graph, filter_representation, sampled_edge_type).squeeze(dim=-1)
            negative_score = model[1](negative_graph, filter_representation, sampled_edge_type).squeeze(dim=-1)
...
if __name__ == '__main__':
    ...
    # todo
    graph, _, _, _ = load_patent_dataset(data_path=args['data_path'],
                                        predict_category=args['predict_category'],
                                        data_split_idx_path=args['data_split_idx_path'])

    if args['add_patent_neighbor']:
        graph = add_neighbor(graph, args['sampled_edge_type'])
        print("generate the new graph successfully!")

    reverse_etypes = dict()
    for stype, etype, dtype in graph.canonical_etypes:
        if stype == dtype:
            reverse_etypes[etype] = etype
            continue
    ...
    model = nn.Sequential(hgcn, link_score_predictor)
    ...
    for epoch in range(args['epochs']):
        ...
        for batch, (input_nodes, positive_graph, negative_graph, blocks) in enumerate(train_loader_tqdm):
            ...
            filter_representation = {"applicant": nodes_representation["applicant"], "company": nodes_representation["company"]}

            positive_score = model[1](positive_graph, filter_representation, args['sampled_edge_type']).squeeze(dim=-1)
            negative_score = model[1](negative_graph, filter_representation, args['sampled_edge_type']).squeeze(dim=-1)
