...
from attentive_types_aggregation import attentive_types_aggregation
...
class HGCNLayer(nn.Module):
    def __init__(self, graph: dgl.DGLHeteroGraph, input_dim: int, hidden_dim: int, n_heads: int = 4,
        ...
        # TODO: This is reasonable, make the interaction of heads from same type aggregation module, shape is (n_heads * hidden_dim, n_heads * hidden_dim)
        ...
        # TODO: another choice, shape is (n_heads, hidden_dim, hidden_dim)
        # different types aggregation trainable parameters
        # self.edge_type_transformation_weight = nn.ParameterDict({
        ...
        self.hetero_conv = HeteroGraphConv({
            etype: GATConv(in_feats=(input_dim, input_dim), out_feats=hidden_dim,
                           num_heads=n_heads, dropout=dropout, negative_slope=0.2)
            for srctype, etype, dsttype in graph.canonical_etypes
        })

        # different types aggregation module
        self.types_aggregation = attentive_types_aggregation(in_feats=hidden_dim * n_heads, out_feats=hidden_dim,
                                                             num_heads=n_heads,
    ...
    def forward(self, graph: dgl.DGLHeteroGraph, node_features: dict):
        ...
        output_features = self.types_aggregation(graph, input_dst, relation_features,
...
class HGCN(nn.Module):
    def __init__(self, graph: dgl.DGLHeteroGraph, input_dim_dict: dict, hidden_dim: int, num_layers: int, n_heads: int = 4,
        ...
        for _ in range(self.num_layers):
            self.layers.append(HGCNLayer(graph, hidden_dim * n_heads, hidden_dim, n_heads, dropout, residual))
