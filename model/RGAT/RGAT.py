...
class RelGraphConvLayer(nn.Module):
    def __init__(self, graph: dgl.DGLHeteroGraph, in_feat, out_feat, num_heads, num_bases, dropout=0.0, activation=None,
                 self_loop=False):
        super(RelGraphConvLayer, self).__init__()
        self.in_feat = in_feat
        self.out_feat = out_feat*num_heads
        ...
        self.relation_conv = dglnn.HeteroGraphConv({
            ...
            etype: dglnn.GATConv(in_feat, self.out_feat//num_heads, num_heads=num_heads) for etype in self.etypes
        })
        ...
        if self.use_basis:
            ...
            self.basis = dglnn.WeightBasis((in_feat, self.out_feat), num_bases, len(self.etypes))
        else:
            # shape -> (rel_names, in_feat, out_feat)
            self.weight = nn.Parameter(torch.randn(len(self.etypes), in_feat, self.out_feat))
            # print(self.weight.shape)

        self.h_bias = nn.Parameter(torch.randn(self.out_feat))

        # weight for self loop
        if self.self_loop:
            self.loop_weight = nn.Parameter(torch.randn(in_feat, self.out_feat))
    ...
    def forward(self, graph: dgl.DGLHeteroGraph, node_features: dict):
        ...
        weight_dict = {self.etypes[i]: {'weight': w.squeeze(dim=0)} for i, w in
                       enumerate(torch.split(weight, split_size_or_sections=1, dim=0))}

        # output_features = self.relation_conv(graph, (input_src, input_dst), mod_kwargs=weight_dict)
        output_features = self.relation_conv(graph, (input_src, input_dst))

        dst_node_features = {}
        for dtype in output_features:
            h = output_features[dtype].reshape(-1, self.out_feat)
            # print(h.shape, input_dst[dtype].shape, self.loop_weight.shape)
            if self.self_loop:
...
class RGAT(nn.Module):
    def __init__(self, graph: dgl.DGLHeteroGraph, input_dim_dict: dict, hidden_sizes: list, num_heads: int,
                 num_bases: int, dropout=0.0, use_self_loop=False):
        """
        ...
        self.layers.append(
            RelGraphConvLayer(graph, self.hidden_sizes[0], self.hidden_sizes[0], num_heads=num_heads,
        ...
        for index in range(1, len(self.hidden_sizes)):
            self.layers.append(
                RelGraphConvLayer(graph, self.hidden_sizes[index - 1], self.hidden_sizes[index], num_heads=num_heads,
